 ...
 */
package org.jtalks.jcommune.model.utils.jsonresponse;

import com.google.common.collect.ImmutableMap;
import org.jtalks.jcommune.model.utils.Users;
import org.jtalks.jcommune.model.utils.assertion.JsonResponseResultAssertor;
 ...
 */
public class JsonResponseUsers extends Users {
    @Override
    ...
    @Override
    public ResultAssertor<String> signUpWithPasswordAndConfirmation(String password, String confirmation)
            throws Exception {
        return buildDefaultResultAssertor(signUp(USERNAME, EMAIL, password, confirmation, HONEYPOT), USERNAME);
    }
    ...
    public ResultAssertor<String> signUpWithHoneypot(String honeypot) throws Exception {
        return buildDefaultResultAssertor(signUp(USERNAME, EMAIL, PASSWORD, CONFIRMATION, honeypot), USERNAME)
                .withFailJsonResponse(new JsonResponse(JsonResponseStatus.FAIL,
                        new ImmutableMap.Builder<String, String>().put(UserController.CUSTOM_ERROR,
                                UserController.HONEYPOT_CAPTCHA_ERROR).build()));
    }
    ...
    @Override
    protected ResultActions signUp(String username, String email, String password, String confirmation, String honeypot)
            throws Exception {
        return getMockMvc().perform(post("/user/new_ajax")
    ...
    private JsonResponseResultAssertor<String> buildDefaultResultAssertor(ResultActions actions,
                                                                          String entityIdentifier) {
        return JsonResponseResultAssertor.fromResultActions(actions)
                .withEntityIdentifier(entityIdentifier)
                .withSuccessStatusMatcher(status().isOk())
                .withSuccessJsonResponse(new JsonResponse(JsonResponseStatus.SUCCESS))
